...
private:
    ...
    void handleCgiExtDirective(std::map<std::string, std::string>& directives, const std::string& line);
    void handleKeyOnlyDirective(std::map<std::string, std::string>& directives, const std::string& trimmed_line, size_t first_char);
    void handleKeyValueDirective(std::map<std::string, std::string>& directives, const std::string& trimmed_line, size_t first_char, size_t first_space);
    bool parseListenDirective(ServerBlock& server, const std::string& value);
    void parseServerNameDirective(ServerBlock& server, const std::string& value);
    bool parseMaxBodySizeDirective(ServerBlock& server, const std::string& value);
    bool parseErrorPageDirective(ServerBlock& server, const std::string& value);
    bool processServerDirective(ServerBlock& server, const std::string& key, const std::string& value);
    bool validateLocationPath(const std::string& line, std::string& path);
    bool findLocationOpeningBrace(std::istream& config_file, const std::string& line, size_t& brace_pos);
    ...
    void parseCgiDirective(RouteConfig& route, const std::string& key, const std::string& value);
    void parseUploadDirective(RouteConfig& route, const std::string& key, const std::string& value);
    bool processLocationDirective(RouteConfig& route, const std::string& key, const std::string& value);
