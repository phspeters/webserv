...
struct ServerConfig {
    ...
    std::string default_index;  // Default file to serve for directory requests
                                // (e.g., "index.html")
    ...
    std::map<std::string, std::string>
        cgi_executables;  // Maps URI path/extension to CGI script path (e.g.,
                          // ".php" -> "/usr/bin/php-cgi")
    ...
    //--------------------------------------
    bool loadFromFile(const std::string& filename);  // Declaration for loading
                                                     // logic (impl in .cpp)
    std::string getMimeType(
        const std::string& filename) const;  // Declaration (impl in .cpp)
