...
void ErrorHandler::bad_request(HttpResponse* resp) { handle_error(resp, 400); }

void ErrorHandler::unauthorized(HttpResponse* resp) { handle_error(resp, 401); }

void ErrorHandler::forbidden(HttpResponse* resp) { handle_error(resp, 403); }

void ErrorHandler::not_found(HttpResponse* resp) { handle_error(resp, 404); }
...
std::string ErrorHandler::get_error_page_content(
    int status_code, const VirtualServer* virtual_server) {
    // Check if custom error page is configured
    std::map<int, std::string>::const_iterator it =
        virtual_server->error_pages_.find(status_code);

    if (it != virtual_server->error_pages_.end()) {
