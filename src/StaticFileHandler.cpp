...
bool StaticFileHandler::process_directory_redirect(Connection* conn, std::string& absolute_path) {
...
bool StaticFileHandler::process_directory_index(Connection* conn, std::string& absolute_path, bool& need_autoindex) {
...
void StaticFileHandler::generate_directory_listing(Connection* conn, const std::string& dir_path) {
    
    // Open the directory
    DIR* dir = opendir(dir_path.c_str());
    // Fluxogram 500 - request resource not found - call error handler
    if (!dir) {
        std::cerr << "Failed to open directory for listing: " << strerror(errno) << std::endl;
        conn->response_data_->status_code_ = 500;
        conn->response_data_->status_message_ = "Internal Server Error";
        // conn->state_ = Connection::CONN_WRITING;
        return;
    ...
    std::string html = "<!DOCTYPE html>\n"
                      "<html>\n"
                      "<head>\n"
                      "    <title>Index of " + conn->request_data_->uri_ + "</title>\n"
                      "    <style>\n"
                      "        body { font-family: Arial, sans-serif; margin: 0; padding: 20px; color: #333; }\n"
                      "        h1 { border-bottom: 1px solid #eee; padding-bottom: 10px; font-size: 24px; }\n"
                      "        table { border-collapse: collapse; width: 100%; }\n"
                      "        th { text-align: left; padding: 8px; border-bottom: 1px solid #ddd; color: #666; }\n"
                      ...
                      "</head>\n"
                      "<body>\n"
                      "    <h1>Index of " + conn->request_data_->uri_ + "</h1>\n";
